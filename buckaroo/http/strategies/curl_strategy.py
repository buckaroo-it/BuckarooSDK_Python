...
class CurlStrategy(HttpStrategy):
        ...
        for attempt in range(self._retry_attempts):
            try:
                ...
                last_exception = Exception(
                    f"Request timeout after {timeout or self._timeout} seconds"
                )
    ...
    def _parse_curl_output(self, result: subprocess.CompletedProcess) -> HttpResponse:
        ...
        if not output:
            ...
            return HttpResponse(
                status_code=result.returncode, headers={}, text="", success=result.returncode == 0
            )
        ...
        return HttpResponse(
            status_code=status_code, headers=headers, text=body, success=200 <= status_code < 300
        )
