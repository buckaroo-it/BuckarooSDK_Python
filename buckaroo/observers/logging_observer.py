...
class BuckarooLoggingObserver:
    ...
    def log_request(
        ...
        **kwargs,
        ...
        log_message = [f"HTTP REQUEST [{request_id}]", f"Method: {method}", f"URL: {url}"]
    ...
    def log_response(
        ...
        **kwargs,
        ...
        log_message = [f"HTTP RESPONSE [{request_id}]", f"Status: {status_code}"]
    ...
    def log_exception(
        self, exception: Exception, context: Optional[Dict[str, Any]] = None, **kwargs
    ):
        """
...
class ContextualLoggingObserver:
    ...
    def log_payment_operation(
        ...
        **kwargs,
        ...
        merged_kwargs = {**self.context, **kwargs}
        self.parent.log_payment_operation(
            operation, payment_method, amount, currency, **merged_kwargs
        )
...
def create_logger(
    ...
    **kwargs,
    ...
    """
    config = LogConfig(level=level, destination=destination, log_file=log_file, **kwargs)
...
def create_logger_from_env() -> BuckarooLoggingObserver:
    ...
    config = LogConfig(
        level=level, destination=destination, log_file=log_file, mask_sensitive_data=mask_sensitive
    )
