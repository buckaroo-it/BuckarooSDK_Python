...
class TestCreditcardFeature:
    ...
    def test_creditcard_pay(self, buckaroo, mock_strategy):
        response_body = Helpers.pending_redirect_response("creditcard", "Pay")
    ...
    def test_creditcard_authorize(self, buckaroo, mock_strategy):
        response_body = Helpers.pending_redirect_response("creditcard", "Authorize")
        mock_strategy.queue(BuckarooMockRequest.json("POST", "*/json/transaction", response_body))
        response = buckaroo.payments.create_payment(
            "creditcard",
            Helpers.standard_payload(
    ...
    def test_creditcard_pay_encrypted(self, buckaroo, mock_strategy):
        response_body = Helpers.pending_redirect_response("creditcard", "PayEncrypted")
        mock_strategy.queue(BuckarooMockRequest.json("POST", "*/json/transaction", response_body))
    ...
    def test_creditcard_pay_with_token(self, buckaroo, mock_strategy):
        response_body = Helpers.pending_redirect_response("creditcard", "PayWithToken")
        mock_strategy.queue(BuckarooMockRequest.json("POST", "*/json/transaction", response_body))
        builder = buckaroo.payments.create_payment(
            "creditcard",
            Helpers.standard_payload(
    ...
    def test_creditcard_authorize_encrypted(self, buckaroo, mock_strategy):
        response_body = Helpers.pending_redirect_response("creditcard", "AuthorizeEncrypted")
        mock_strategy.queue(BuckarooMockRequest.json("POST", "*/json/transaction", response_body))
        builder = buckaroo.payments.create_payment(
            "creditcard",
            Helpers.standard_payload(
    ...
    def test_creditcard_refund_sends_action_refund_on_the_wire(
        ...
        recording_mock.queue(
            BuckarooMockRequest.json(
                "POST",
                "*/json/transaction*",
                Helpers.refund_response("creditcard"),
            )
        )
    ...
    def test_creditcard_authorize_sends_action_authorize_on_the_wire(
        ...
        recording_mock.queue(
            BuckarooMockRequest.json(
                "POST",
                "*/json/transaction*",
                Helpers.pending_redirect_response("creditcard", "Authorize"),
            )
        )
        recording_buckaroo.payments.create_payment(
            "creditcard",
            Helpers.standard_payload(invoice="INV-CC-WIRE-AUTH"),
        ).authorize()
        ...
        recording_mock.queue(
            BuckarooMockRequest.json(
                ...
                Helpers.success_response(
                    {
                        "Services": [
                            {"Name": "creditcard", "Action": "CancelAuthorize", "Parameters": []}
                        ],
                        "ServiceCode": "creditcard",
                    }
                ),
    ...
    def test_creditcard_pay_encrypted_sends_action_payencrypted_on_the_wire(
        ...
        recording_mock.queue(
            BuckarooMockRequest.json(
                "POST",
                "*/json/transaction*",
                Helpers.pending_redirect_response("creditcard", "PayEncrypted"),
            )
        )
        builder = recording_buckaroo.payments.create_payment(
            "creditcard",
            Helpers.standard_payload(invoice="INV-CC-WIRE-ENC"),
        )
        builder.add_parameter("EncryptedCardData", "encrypted-data-here")
