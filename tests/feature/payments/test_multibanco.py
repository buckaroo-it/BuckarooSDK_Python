...
class TestMultibancoFeature:
    def test_multibanco_pay_returns_pending_with_redirect(self, buckaroo, mock_strategy):
        response_body = TestHelpers.pending_redirect_response("multibanco")
        mock_strategy.queue(BuckarooMockRequest.json("POST", "*/json/transaction", response_body))

        response = buckaroo.payments.create_payment("multibanco", TestHelpers.standard_payload(
            invoice="INV-MB-001",
            description="Test multibanco",
        )).pay()
