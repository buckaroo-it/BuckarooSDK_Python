...
class TestPayconiqFeature:
    ...
    def test_payconiq_refund(self, buckaroo, mock_strategy):
        TestHelpers.assert_refund_returns_success(
            buckaroo, mock_strategy, method="payconiq", invoice="INV-PCQ-REFUND",
        )

    def test_payconiq_instant_refund(self, buckaroo, mock_strategy):
        TestHelpers.assert_instant_refund_returns_success(
            buckaroo, mock_strategy, method="payconiq", invoice="INV-PCQ-IREFUND",
        )
    ...
    def test_payconiq_fast_checkout(self, buckaroo, mock_strategy):
        TestHelpers.assert_fast_checkout_returns_pending_with_redirect(
            buckaroo, mock_strategy, method="payconiq", invoice="INV-PCQ-FAST",
        )
