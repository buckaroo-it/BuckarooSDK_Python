...
class TestSwishFeature:
    def test_swish_pay_returns_pending_with_redirect(self, buckaroo, mock_strategy):
        response_body = TestHelpers.pending_redirect_response("swish")
        mock_strategy.queue(
            BuckarooMockRequest.json("POST", "*/json/transaction", response_body)
        )
        response = buckaroo.payments.create_payment("swish", TestHelpers.standard_payload(
            invoice="INV-SWI-001",
            description="Test swish",
        )).pay()
