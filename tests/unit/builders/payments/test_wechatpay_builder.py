...
# ---------------------------------------------------------------------------
# Capability mixin sanity — WeChatPay mixes in nothing
...
def test_wechatpay_builder_does_not_inherit_capability_mixin(capability):
    assert not issubclass(WeChatPayBuilder, capability), (
        f"WeChatPayBuilder unexpectedly inherits {capability.__name__}; "
        "WeChatPay does not support that capability per the SDK registry."
    )
...
def test_base_builder_methods_present_and_callable(client, method):
    """WeChatPayBuilder inherits every generic action from BaseBuilder."""
    builder = WeChatPayBuilder(client)
